...
- **统计视图（StatsScreen）加「模式」筛选分段：全部 / 认音 / 跟弹**，作用于 **速度趋势与最近准确率**（两条由 `listSessions()` 记录驱动；认音逐键准确率与跟弹首击准确率本不可比，混在一起曲线失真）；缺省「全部」向后兼容。聚合函数加 mode 过滤入参（纯函数可测）。**常错音符不按模式拆分**：错音池本就跨模式共享（同步也是加权复习的输入），`progress.wrong` 为全局聚合、无 mode 维度，故该卡始终显示全部错音。打卡区不设筛选。
